...
public class GenomeNexusImpl implements Annotator {
    ...
    public AnnotatedRecord convertResponseToAnnotatedRecord(VariantAnnotation gnResponse, MutationRecord mRecord, boolean replace) {
        ...
        AnnotatedRecord annotatedRecord = new AnnotatedRecord(annotationUtil.resolveHugoSymbol(canonicalTranscript, mRecord, replace),
                annotationUtil.resolveEntrezGeneId(canonicalTranscript, mRecord, replace), mRecord.getCENTER(), annotationUtil.resolveAssemblyName(gnResponse, mRecord),
                annotationUtil.resolveChromosome(gnResponse, mRecord), annotationUtil.resolveStart(gnResponse, mRecord), annotationUtil.resolveEnd(gnResponse, mRecord),
                annotationUtil.resolveStrandSign(gnResponse, mRecord), annotationUtil.resolveVariantClassification(canonicalTranscript, mRecord),
                annotationUtil.resolveVariantType(gnResponse), resolvedReferenceAllele, resolvedTumorSeqAllele1, resolvedTumorSeqAllele2,
                annotationUtil.resolveDbSnpRs(gnResponse, mRecord), mRecord.getDBSNP_VAL_STATUS(), mRecord.getTUMOR_SAMPLE_BARCODE(), mRecord.getMATCHED_NORM_SAMPLE_BARCODE(),
                mRecord.getMATCH_NORM_SEQ_ALLELE1(), mRecord.getMATCH_NORM_SEQ_ALLELE2(), mRecord.getTUMOR_VALIDATION_ALLELE1(), mRecord.getTUMOR_VALIDATION_ALLELE2(),
                mRecord.getMATCH_NORM_VALIDATION_ALLELE1(), mRecord.getMATCH_NORM_VALIDATION_ALLELE2(), mRecord.getVERIFICATION_STATUS(), mRecord.getVALIDATION_STATUS(),
                mRecord.getMUTATION_STATUS(), mRecord.getSEQUENCING_PHASE(), mRecord.getSEQUENCE_SOURCE(), mRecord.getVALIDATION_METHOD(), mRecord.getSCORE(),
                mRecord.getBAM_FILE(), mRecord.getSEQUENCER(), mRecord.getTUMOR_SAMPLE_UUID(), mRecord.getMATCHED_NORM_SAMPLE_UUID(), mRecord.getT_REF_COUNT(),
                mRecord.getT_ALT_COUNT(), mRecord.getN_REF_COUNT(), mRecord.getN_ALT_COUNT(), annotationUtil.resolveHgvsc(canonicalTranscript),
                annotationUtil.resolveHgvsp(canonicalTranscript), annotationUtil.resolveHgvspShort(canonicalTranscript), annotationUtil.resolveTranscriptId(canonicalTranscript),
                annotationUtil.resolveRefSeq(canonicalTranscript), annotationUtil.resolveProteinPosStart(canonicalTranscript),
                annotationUtil.resolveProteinPosEnd(canonicalTranscript), annotationUtil.resolveCodonChange(canonicalTranscript), annotationUtil.resolveHotspot(),
                annotationUtil.resolveConsequence(canonicalTranscript), annotationUtil.resolveProteinPosition(canonicalTranscript, mRecord),
                annotationUtil.resolveExon(canonicalTranscript), mRecord.getAdditionalProperties());
        ...
        if (enrichmentFields.contains("my_variant_info")) {
            ...
            annotatedRecord.setGnomadFields(annotationUtil.resolveGnomadAlleleFrequency(alleleFrequency), annotationUtil.resolveGnomadAlleleFrequencyAFR(alleleFrequency),
                    annotationUtil.resolveGnomadAlleleFrequencyAMR(alleleFrequency), annotationUtil.resolveGnomadAlleleFrequencyASJ(alleleFrequency),
                    annotationUtil.resolveGnomadAlleleFrequencyEAS(alleleFrequency), annotationUtil.resolveGnomadAlleleFrequencyFIN(alleleFrequency),
                    annotationUtil.resolveGnomadAlleleFrequencyNFE(alleleFrequency), annotationUtil.resolveGnomadAlleleFrequencyOTH(alleleFrequency),
                    annotationUtil.resolveGnomadAlleleFrequencySAS(alleleFrequency));
        }
        if (enrichmentFields.contains("polyphen")) {
            annotatedRecord.setPolyphenFields(annotationUtil.resolvePolyphenPrediction(canonicalTranscript), annotationUtil.resolvePolyphenScore(canonicalTranscript));
        }
        if (enrichmentFields.contains("sift")) {
            annotatedRecord.setSiftFields(annotationUtil.resolveSiftPrediction(canonicalTranscript), annotationUtil.resolveSiftScore(canonicalTranscript));
        }
        if (enrichmentFields.contains("mutation_assessor")) {
            annotatedRecord.setMutationAssessorFields(annotationUtil.resolveMaFunctionalImpact(gnResponse), annotationUtil.resolveMaFunctionalImpactScore(gnResponse),
                    annotationUtil.resolveMaLinkMSA(gnResponse), annotationUtil.resolveMaLinkPDB(gnResponse));
        }
        if (enrichmentFields.contains("nucleotide_context")) {
            annotatedRecord.setNucleotideContextFields(annotationUtil.resolveRefTri(gnResponse), annotationUtil.resolveVarTri(gnResponse));
        }
    ...
    public GenomicLocation parseGenomicLocationFromRecord(MutationRecord record) {
        ...
        genomicLocation.setVariantAllele(MafUtil.resolveTumorSeqAllele(record.getREFERENCE_ALLELE(), record.getTUMOR_SEQ_ALLELE1(), record.getTUMOR_SEQ_ALLELE2()));
        return genomicLocation;
    }

    public String parseGenomicLocationString(MutationRecord mutationRecord) {
        GenomicLocation genomicLocation = parseGenomicLocationFromRecord(mutationRecord);
        return parseGenomicLocationString(genomicLocation);
    }

    public String parseGenomicLocationString(GenomicLocation genomicLocation) {
    ...
    @Override
    public List<AnnotatedRecord> getAnnotatedRecordsUsingPOST(final AnnotationSummaryStatistics summaryStatistics, final List<MutationRecord> mutationRecords,
                                                              final String isoformOverridesSource, final Boolean replace, final Integer postIntervalSize,
        ...
        Map<String, MutationRecord> genomicLocationToMutationRecord = new HashMap<>();
        for (MutationRecord record : mutationRecords) {
            GenomicLocation genomicLocation = parseGenomicLocationFromRecord(record);
            genomicLocationToMutationRecord.put(parseGenomicLocationString(genomicLocation), record);
            if (reannotate || annotationNeeded(record)) {
                genomicLocations.add(genomicLocation);
            }
        }
        if (genomicLocations.size() != genomicLocationToMutationRecord.size()) {
            LOG.warn("genomicLocations.size() != genomicLocationToMutationRecord.size()");
        }
        ...
        for (List<GenomicLocation> partitionedList : partitionedGenomicLocationList) {
            ...
            for (GenomicLocation genomicLocation : partitionedList) {
                String genomicLocationString = parseGenomicLocationString(genomicLocation);
                MutationRecord record = genomicLocationToMutationRecord.get(genomicLocationString);
                AnnotatedRecord annotatedRecord = new AnnotatedRecord(record);
                // if not a failed annotation then convert/merge the response from gn with the maf record
                VariantAnnotation gnResponse = gnResponseVariantKeyMap.get(genomicLocationString);
                if (gnResponse == null) {
                    if (reannotate || annotationNeeded(record)) {
                    ...
                    annotatedRecord = convertResponseToAnnotatedRecord(gnResponse, record, replace);
                    annotatedRecord.setANNOTATION_STATUS(
                            "SUCCESS"); // annotation status indicates if a response was returned from GN, not whether the annotation is considered valid or not
